...
class SearchStores extends Component
{
    ...
    public function render(){
        ...
        if($product_search == ''){
            $stores = $this->queryStoresNotFound($categories_id, $cities_id);
            $search_empty = true;
        }else{
    ...
    public function queryStoresNotFound($categories_id, $city_id){
        $query = Store::join('product_stores', 'stores.id' , '=' , 'product_stores.stores_id')->join('products', 'products.id' , '=' , 'product_stores.products_id');
        $query = $query->where('product_stores.amount', '>', 0)->where('status', true)->where('cities_id', $city_id)->where('stores.status', true);
    ...
    public function queryDataCity($categories_id, $product_search, $city_id){        
        $query = Store::join('product_stores', 'stores.id' , '=' , 'product_stores.stores_id')->join('products', 'products.id' , '=' , 'product_stores.products_id');
        $query = $query->whereFullText('products.name', $product_search)->where('product_stores.amount', '>', 0)->where('status', true)->where('cities_id', $city_id)->where('stores.status', true);
        if($categories_id != 'Categoria'){
            $category = Category::find($categories_id);
            if($category->subCategories->isNotEmpty()){
                $id_sub_category = SubCategory::where('categories_id', $categories_id)->select('id')->first()->id;
    ...
    public function queryDataState($categories_id, $product_search, $state_id){
        ...
        $query = $query->whereFullText('products.name', $product_search)->where('product_stores.amount', '>', 0)->where('status', true)->where('municipalities.states_id', $state_id)->where('stores.status', true);
        if($categories_id != 'Categoria'){
            $category = Category::find($categories_id);
            if($category->subCategories->isNotEmpty()){
                $id_sub_category = SubCategory::where('categories_id', $categories_id)->select('id')->first()->id;
    ...
    public function queryDataCountry($categories_id, $product_search, $country_id){
        ...
        $query = $query->whereFullText('products.name', $product_search)->where('product_stores.amount', '>', 0)->where('status', true)->where('countries.id', $country_id)->where('stores.status', true);
        if($categories_id != 'Categoria'){
            $category = Category::find($categories_id);
            if($category->subCategories->isNotEmpty()){
                $id_sub_category = SubCategory::where('categories_id', $categories_id)->select('id')->first()->id;
