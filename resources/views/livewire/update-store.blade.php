        ...
        <form id="form2">
            <div class="row">
                <div class="col-md-6 form-group">
                    ...
                    <input type="text" class="form-control mt-3 mb-3" name="name" value="{{ $store->name }}"
                        placeholder="Ingrese un nombre">
                    <label>Dirección</label>
                    <input type="text" class="form-control mt-3 mb-3" name="address" value="{{ $store->address }}"
                        placeholder="Ingrese una dirección">
                    <label>Descripción</label>
                    <input type="text" class="form-control mt-3 mb-3" name="description"
                ...
                <div class="col-md-6 form-group">
                    ...
                    <input type="email" class="form-control mt-3 mb-3" name="email" value="{{ $store->email }}"
                        placeholder="Ingrese un correo">
                    <label>Numeros de contacto</label>
                    <input type="number" class="form-control mt-3" name="phone" value="{{ $store->phone }}"
                        placeholder="Ingrese un numero">
                    @if ($store->typeStore->description == env('TIPO_GRUA'))
                        <label class="mt-3">Capacidad de la grúa</label>
                        <input type="text" class="form-control mt-3" name="capacidad" value="{{ $store->capacidad }}"
                            placeholder="Ingrese un numero">
                        <label class="mt-3">Dimensiones de la grúa</label>
                        <input type="text" class="form-control mt-3" name="dimensiones"
                            value="{{ $store->dimensiones }}" placeholder="Ingrese las dimensiones">
                    @endif
...
@section('js')
    ...
    @endif
    <script>
        var isset_images4 = false;

        var myDropzone4 = new Dropzone("#myDropzone92", {
            ...
            init: function() {
                this.on("sending", function(file, xhr, formData) {
                    formData.append("id", $("#id_publicities_save").val());
                    formData.append("table", `publicities`);
                });
        ...
        function storePublicity() {
            $.ajax({
                url: "{{ route('table-store-imgs-5') }}",
                data: $("#form-publicity").serialize(),
                method: "POST",
                headers: {
                    'X-CSRF-TOKEN': "{{ csrf_token() }}",
                },
                success(response) {
                    ...
                    $("#id_publicities_save").val(res.split('-')[1]);
                    myDropzone4.processQueue();
                    setTimeout(() => {
                        if (isset_images4 == false) {
                            hideAlertTime();
                        }
                    }, 3000);
                },
